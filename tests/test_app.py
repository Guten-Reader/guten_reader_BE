...
class TestHello(unittest.TestCase):
    ...
    def test_get_watson(self, mock_text_sentiment):
        ...
        mock_text_sentiment.return_value = MagicMock(status_code=200,response=ml_data)
        ...
        self.assertEqual(200, response.status_code)
        self.assertDictEqual(ml_data, response.json)


    @patch('services.spotify_service.requests.get')
    @patch.object(WatsonService, 'get_sentiment_value')
    def test_POST_recommendation(self, mock_text_sentiment, mock_get):
        ...
        mock_text_sentiment.return_value = ml_data
        # mock_text_sentiment.return_value.status_code = 200
